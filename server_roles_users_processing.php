...
$aclrole_list = $db->query("select * from  acl_roles_users where role_id='".$_GET['role_id']."' and deleted='0'");
		while ($aclrolerow = $db->fetchByAssoc($aclrole_list)) {
		
		$result = $db->query("select * from users where id='".$aclrolerow['user_id']."' and deleted='0' and id!='".$_GET['copy_from']."'");
...
.reports_to_id=users.id) AS report_name, CONCAT( COALESCE(u.first_name,''), ' ', COALESCE(u.last_name,'') ) AS name,  u.id AS id, u.title AS title, u.department AS department, u.reports_to_id AS reports_to_id, ea.email_address AS email FROM user_preferences AS up LEFT JOIN users AS u ON u.id = up.assigned_user_id LEFT JOIN email_addr_bean_rel AS ebr ON ebr.bean_id = u.id LEFT JOIN email_addresses AS ea ON ea.id = ebr.email_address_id where ".$sWhere." u.deleted = '0' AND ebr.bean_module = 'Users' AND ebr.deleted = '0' AND u.id IN($imp_id) ".$sOrder." ".$sLimit.""; 
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		$row = array();
		//$row[]="";
		if($_GET['role_user_selected']=="all_selected")
		{
		$row[]="<input type='checkbox' name='aclrole_list[]' class='aclrole_checkbox' value='".$aRow['id']."' id='aclrole_list_".$aRow['id']."' checked='checked' disabled='disabled'>";		
		}else
		{
		$row[]="<input type='checkbox' name='aclrole_list[]' value='".$aRow['id']."' class='aclrole_checkbox' id='aclrole_list_".$aRow['id']."'>";	
		}
