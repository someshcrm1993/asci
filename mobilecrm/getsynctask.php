...
$sql2 = "SELECT IFNULL( ta.id, '' ) AS id, IFNULL( ta.name, '' ) AS name,

         IFNULL( ta.status, '' ) AS status,IFNULL( ta.parent_type, '' ) AS parent_type,
         IFNULL( ta.parent_id, '' ) AS parent_id,IFNULL( ta.date_start, '' ) AS date_start,
         IFNULL( ta.date_due, '' ) AS date_due,IFNULL( ta.priority, '' ) AS priority,
         IFNULL( ta.description, '' ) AS description,IFNULL( ta.contact_id, '' ) AS contact_id,
         IFNULL( ta.assigned_user_id, '' ) AS assigned_user_id, ta.date_entered as date_entered,
         ta.date_modified as date_modified, IFNULL( u.user_name, '' ) AS user_name, 
         CONCAT( IFNULL( u.first_name, '' ) , ' ', IFNULL( u.last_name, '' ) ) AS assigned_user_full_name

FROM tasks AS ta
LEFT JOIN tasks_cstm AS tac ON ta.id = tac.id_c
LEFT JOIN users AS u ON ta.assigned_user_id = u.id
WHERE ta.assigned_user_id = '$assigned_user_id' AND ta.date_modified >=  '$date_modified_sugar_format_sync' order by ta.date_modified DESC";
