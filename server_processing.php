	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if ( $aColumns[$i] == "id" )
			{
				...
				$row[]="<button class='btn btn-primary btn-sm view-dashboard-button' id='view_dashboard_".$aRow[$aColumns[$i]]."' name='view_dashboard_".$aRow[$aColumns[$i]]."' data-toggle='modal' data-target='#view_dashboard_modal' >Copy to</button>";
