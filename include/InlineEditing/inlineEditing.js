...
function setValueClose(value){
	...
	for(var i = 0; i<svgElement.length; i++){
		var sa = svgElement[i].getAttribute('id');
		if(sa == 'inline_edit_icon')
			break;
	}
	var svgElement = $('svg')[i];
	var  inlineEdit= $('<div>').append($(svgElement).clone()).html();
        $(".inlineEditActive").html("");
        $(".inlineEditActive").html(value + '<div class="inlineEditIcon">' + inlineEdit + '</div>');
