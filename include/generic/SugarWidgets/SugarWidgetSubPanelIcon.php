...
/* ------------ Subpanel activities and history module image icon replace with fa icons start-------Roshan Sarode-------*/
		
		if($module=="Meetings")
		{
			$icon_img_html='<i class="fa fa-users fa-stack-1x" aria-hidden="true"></i>';
		
		}else if($module=="Tasks")
		{
			$icon_img_html='<i class="fa fa-tasks fa-stack-1x" aria-hidden="true"></i>';
		}
		else if($module=="Calls")
		{
		    $icon_img_html='<i class="fa fa-phone fa-stack-1x" aria-hidden="true"></i>';
		}
		else if($module=="Emails")
		{
			$icon_img_html='<i class="fa fa-envelope fa-stack-1x" aria-hidden="true"></i>';
		}
		else if($module=="Notes")
		{
			$icon_img_html='<i class="fa fa-sticky-note fa-stack-1x" aria-hidden="true"></i>';
		}else
		{
		$icon_img_html = SugarThemeRegistry::current()->getImage( $module . '', 'border="0" ',null,null,'.gif',$app_list_strings['moduleList'][$module]);
			
		}
/* ------------ Subpanel activities and history module image icon replace with fa icons end-------Roshan Sarode-------*/		
		...
		if (!empty($layout_def['attachment_image_only']) && $layout_def['attachment_image_only'] == true) {
			...
			if ($action_access) {
/* ------------ Subpanel activities and history module image icon replace with fa icons start-------Roshan Sarode-------*/				
				$ret = '<a href="index.php?module=' . $module . '&action=' . $action . '&record=' . $record	. '"   style="border:1px solid #d9dada; border-radius:30px;margin-right:5px;position: relative;
    display: inline-block;width: 3em;height: 3em;line-height: 3em;vertical-align: middle;margin: 5px;}">' .$icon_img_html . "</a>";
/* ------------ Subpanel activities and history module image icon replace with fa icons end-------Roshan Sarode-------*/			
			} else {
				$ret = $icon_img_html;
			}
