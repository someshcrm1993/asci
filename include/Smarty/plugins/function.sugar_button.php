...
function smarty_function_sugar_button($params, &$smarty)
{
   ...
   if(!is_array($type)) {
   	  ...
   	  switch(strtoupper($type)) {
			case "SEARCH":
                $output = '<button tabindex="2" data-toggle="tooltip" data-placement="bottom" title="{$APP.LBL_SEARCH_BUTTON_TITLE}" onclick="SUGAR.savedViews.setChooser();" class="btn btn-outline-primary" type="submit" name="button" id="search_form_submit"/><i class="fa fa-search fa-lg btn-color-new" aria-hidden="true" ></i></button>';
                //~ $output = '<button tabindex="2" onclick="SUGAR.savedViews.setChooser();" class="btn btn-outline-primary" type="submit" name="button" id="search_form_submit"/><span class="fa fa-search" style="color:#FFF"></span>{$APP.LBL_SEARCH_BUTTON_TITLE}</button>&nbsp;';
			break;
			...
			case "SUBPANELSAVE":
                ...
                */
            if($params['module']=="Tasks" || $params['module']=="Notes" || $params['module']=="Calls" || $params['module']=="Meetings")
			{

			//$output = '{if $bean->aclAccess("save")}<input title="{$APP.LBL_SAVE_BUTTON_TITLE}"  class="button" onclick="'.$js_form.' var _form = document.getElementById(\'EditView\'); form.action.value=\'Save\'; if(check_form(\''.$view.'\'))SUGAR.ajaxUI.submitForm(_form);return false;" type="submit" name="' . $params['module'] . '_subpanel_save_button" id="' . $params['module'] . '_subpanel_save_button" value="{$APP.LBL_SAVE_BUTTON_LABEL}">{/if} ';
			$output = '{if $bean->aclAccess("save")}<input title="{$APP.LBL_SAVE_BUTTON_TITLE}"  class="button" onclick="'.$js_form.'; form.action.value=\'Save\'; if(check_form(\''.$view.'\'))SUGAR.ajaxUI.submitForm(_form);return false;" type="submit" name="' . $params['module'] . '_subpanel_save_button" id="' . $params['module'] . '_subpanel_save_button" value="{$APP.LBL_SAVE_BUTTON_LABEL}">{/if}';
			/* END - SECURITY GROUPS */
			}else
			{
			$output = '{if $bean->aclAccess("save")}<input title="{$APP.LBL_SAVE_BUTTON_TITLE}"  class="button" onclick="'.$js_form.' disableOnUnloadEditView(); _form.action.value=\'Save\';if(check_form(\''.$view.'\'))SUGAR.subpanelUtils.inlineSave(_form.id, \'' . $params['module'] . '_subpanel_save_button\');return false;" type="submit" name="' . $params['module'] . '_subpanel_save_button" id="' . $params['module'] . '_subpanel_save_button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" >{/if} ';                
			}
            break;
			case "SUBPANELCANCEL":
			if($params['module']=="Tasks" || $params['module']=="Notes" || $params['module']=="Calls" || $params['module']=="Meetings")
			{
                //~ $output = '<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" class="subpanel_cancel_button_custom" class="button" onclick="return SUGAR.subpanelUtils.cancelCreate($(this).attr(\'id\'));return false;" type="submit" name="' . $params['module'] . '_subpanel_cancel_button" id="' . $params['module'] . '_subpanel_cancel_button" value="{$APP.LBL_CANCEL_BUTTON_LABEL}" data-dismiss="modal"> ';
                $output = '<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" class="subpanel_cancel_button_custom" class="button" onclick="var _form = document.getElementById(\'EditView\');SUGAR.subpanelUtils.cancelCreate($(this).attr(\'id\'))SUGAR.ajaxUI.submitForm(_form);return false;" type="submit" name="' . $params['module'] . '_subpanel_cancel_button" id="' . $params['module'] . '_subpanel_cancel_button" value="{$APP.LBL_CANCEL_BUTTON_LABEL}" data-dismiss="modal"> ';
