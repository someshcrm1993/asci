...
{{/if}}
{php}
$this->assign('module_delimg',$_REQUEST['module']);
$this->assign('module_id',$_REQUEST['record']);
{/php}
<script type="text/javascript">
    {literal}
    
     function deleteImage(path,id,field,module,module_id) {
      $.ajax({
		url: "imagedelete.php",
		type: "POST",
		data:  {'path':path,'id':id,'field':field,'module':module,'module_id':module_id},
...
<span id="{{$idName}}_old" style="display:{if !$showRemove}none;{/if}">
  <a href="index.php?entryPoint=download&id={$fields.{{$vardef.fileId}}.value}_{{if empty($displayParams.idName)}}{{sugarvar key='name'}}{{else}}{{$displayParams.idName}}{{/if}}&type={{$vardef.linkModule}}&time={$fields.date_modified.value}" class="tabDetailViewDFLink">{{sugarvar key='value'}}</a>
    ...
    {if !$noChange}
       ...
       *}
        <input type='button' class='button' id='remove_button' value='{$APP.LBL_REMOVE}' onclick='deleteImage("upload/","{$fields.{{$vardef.fileId}}.value}","{{$idName}}","{$module_delimg}","{$module_id}");'>
    {/if}
</span>
{if !$noChange}
