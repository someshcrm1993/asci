...
<table id="dashletPanel" cellpadding='0' cellspacing='0' width='100%' border='0' class='list view default'>
 ...
 -->
 							...
 							&nbsp;&nbsp;
								<a id="" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.startPage}")' style="cursor: pointer;" title="{$navStrings.start}">
                        ...
                        {else}
							...
							&nbsp;&nbsp;
							<a id="" disabled='disabled' style="color:#6d6d6d">
								<i class="fa fa-angle-double-left fa-lg" aria-hidden="true"></i>
							</a>
							 &nbsp;&nbsp;
                        {/if}
                        {if $pageData.urls.prevPage}
							...
							&nbsp;&nbsp;
							<a id="" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.prevPage}")' style="cursor: pointer;" title="{$navStrings.previous}">
                        ...
                        {if $pageData.urls.nextPage}
							...
							<!-- <button title='{$navStrings.next}' class='button' onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.nextPage}")'>
							 ...
							 &nbsp;&nbsp;
							<a id="" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.nextPage}")' style="cursor: pointer;" title="{$navStrings.next}">
						...
						{if $pageData.urls.endPage  && $pageData.offsets.total != $pageData.offsets.lastOffsetOnPage}
							...
							<!-- <button title='{$navStrings.end}' class='button' onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.endPage}")'>
							 ...
							 &nbsp;&nbsp;
							<a id="" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.endPage}")' style="cursor: pointer;" title="{$navStrings.end}">
    ...
    <tr height='20'>
        ...
        {foreach from=$displayColumns key=colHeader item=params}
            ...
            {else}<th scope='col' data-hide="{$datahide}">{/if}
				<div style='white-space: normal;font-weight:bold !important' width='100%' align='{$params.align|default:'left'}' >
		...
		<tr height='20' class='{$_rowColor}S1'>
			...
			<td width='1%' class='{$_rowColor}S1' bgcolor='{$_bgColor}' nowrap>
				{if $pageData.rowAccess[$id].edit}
                    ...
                    <a title='{$editLinkString}' href='index.php?action=EditView&module={$pageData.bean.moduleDir}&record={$rowData.ID}&offset={$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}&stamp={$pageData.stamp}&return_module=Home&return_action=index' style='cursor:pointer;color:#545454'>
