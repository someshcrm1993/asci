...
<?php
        ...
        if($showContainer)
        {
            echo '<div class="row" style="border:1px solid #d9dada ;"><ul class="noBullet" id="subpanel_list" >';
        }
        ...
        foreach ($tabs as $tab)
		{
			...
			if($tab!="activities" && $tab!="history"){		//It is for hide activities and history subpanel foreach start	
			...
			echo '<li class="noBullet custom-noBullet" id="whole_subpanel_' . $tab . '">';
            ...
            if($div_display != 'none'){
			    ...
			    $old_contents_new = ob_get_contents();
            	@ob_end_clean();

            	ob_start();
            	include_once('include/SubPanel/SubPanel.php');
            	$subpanel_object_new = new SubPanel($this->module, $_REQUEST['record'], $tab,$thisPanel,$layout_def_key);
            	$subpanel_object_new->setTemplateFile('include/SubPanel/SubPanelCustomHeader.html');
            	$subpanel_object_new->display();
            	$subpanel_data_new = ob_get_contents();
            	@ob_end_clean();
            	
            	ob_start();
            	echo $this->get_buttons($thisPanel,$subpanel_object_new->subpanel_query);
            	$buttons = ob_get_contents();
            	@ob_end_clean();

            	ob_start();
            	echo $old_contents_new;
            	//echo $buttons;
                $display_spd_new = $subpanel_data_new;
	...
	if (empty($this->show_tabs))
			{
				$show_icon_html = SugarThemeRegistry::current()->getImage( 'plus_custom', 'border="0" width="13px" align="absmiddle"',null,null,'.png',translate('LBL_SHOW'));
				$hide_icon_html = SugarThemeRegistry::current()->getImage( 'minus_custom', 'border="0" width="13px" align="absmiddle"',null,null,'.png',translate('LBL_HIDE'));
                ...
                echo ">".get_form_header( $thisPanel->get_title()." ".$display_spd_new, $max_min, false);
                
               // ."<a onclick=\"sub_click('".$tab."','".$layout_def_key."');\">".get_form_header( $thisPanel->get_title(), $max_min, false).'</a>'
			}
...
EOQ;
	...
	}               // activities and history condition is close
        if($showContainer)
        {
        	echo '</ul>
        	</div>';
...
EOQ;
	...
	function get_buttons($thisPanel,$panel_query=null)
	{
        ...
        $widget_contents = smarty_function_sugar_action_menu(array(
            'buttons' => $buttons,
            'class' => 'clickMenu fancymenu custom-size-create',
        ), $this->xTemplate);
