 ...
 function process_dynamic_listview_rows($data,$parent_data, $xtemplateSection, $html_varName, $subpanel_def)
 {
    ...
    while(list($aVal, $aItem) = each($data))
    {
      ...
      $fields['OFFSET'] = ($offset + $count + 1);
        ...
        //echo $subpanel_def->_instance_properties['module'];
        ...
        //$subpanel_data = $this->list_field_defs;
...
//echo "<pre>";
//print_r($thepanel->get_list_fields()['status']['vname']);
...
/* ------------ Subpanel activities and history module start-------Roshan Sarode-------*/
						
						if($subpanel_def->_instance_properties['module']=="Activities" || $subpanel_def->_instance_properties['module']=="History")
						{

						if($list_field['name']=="name")
						{
							
						$this->xTemplate->assign('CELL', "<strong style='font-size:14px;'>".$widget_contents."</strong><br>");
						} else if($list_field['name']=="status")
						{
						$this->xTemplate->assign('CELL', "");
						}else if($list_field['name']=="contact_name")
						{
						//$this->xTemplate->assign('CELL', "Related To : ".$widget_contents."&nbsp;|");

						}else if($list_field['name']=="assigned_user_name")
						{
						$this->xTemplate->assign('CELL',$widget_contents);	
						}
						else if($list_field['name']=="date_modified" )
						{
						$this->xTemplate->assign('CELL', "");	
						}else if($list_field['name']=="date_start")
						{
						...
						//~ {
						//~ $date1=date_create($widget_contents);
						//~ $date2=date_create(date("Y/m/d"));
						//~ $diff=date_diff($date1,$date2);
						//~ $res_days_ago= $diff->format("%a days ago");
							//~ 
						//~ }
						}
						else if($list_field['name']=="date_entered")
						{
						//$this->xTemplate->assign('CELL',$widget_contents);	
						//~ if(!empty($widget_contents))
						//~ {
...
/* ------------ Subpanel activities and history module end-------Roshan Sarode-------*/							
                ...
                $this->xTemplate->parse($xtemplateSection.".row.cell");
                	...
                	if( isset($list_field['widget_class']) && $list_field['widget_class'] == "SubPanelIcon") {
		                ...
		                $this->xTemplate->assign('CLASS', "");
		                if ( empty($widget_contents) ) $widget_contents = '&nbsp;';
		                $this->xTemplate->assign('CELL', "<td><div style='float:left'>".$widget_contents."</div><div style='padding:4px 0px 4px 4px'>");
...
/* ------------ Subpanel activities and history module end-------Roshan Sarode-------*/
        ...
        // the action buttons in a list view. Relevant bugs: #51647 and #51640.
				...
				array_unshift($button_contents, "<a href='javascript:void(0)'></a>");	
				$ext_class="clickMenu subpanel records fancymenu pull-right round-button";
				}else
				{
				$ext_class="clickMenu subpanel records fancymenu ";	
				}	
...
/* ------------ Subpanel activities and history module start--------Roshan Sarode------*/		
					...
					if($subpanel_def->_instance_properties['module']=="Activities" || $subpanel_def->_instance_properties['module']=="History")
					{
					if(!empty($res_days_ago))
					{
					$this->xTemplate->assign('CELL', "&nbsp;|&nbsp;".$res_days_ago."</td><td> ".$action_button);
					}else
					{
					$this->xTemplate->assign('CELL', "</td><td>".$action_button);
					}
					}else
					{
    ...
    function processListNavigation($xtemplateSection, $html_varName, $current_offset, $next_offset, $previous_offset, $row_count, $sugarbean=null, $subpanel_def=null, $col_count = 20) {
        ...
        if(empty($this->query_limit)  || $this->query_limit > $this->records_per_page) {
            ...
            if(0 == $current_offset) {
                ...
                $start_link = "<a name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}'  disabled style='color:#6d6d6d'><i class='fa fa-angle-double-left fa-lg ' aria-hidden='true'></i></a>";
                ...
                $previous_link = "<a name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' disabled style='color:#6d6d6d'><i class='fa fa-angle-left fa-lg ' aria-hidden='true'></i></a>";
            } else {
                if($this->multi_select_popup) {// nav links for multiselect popup, submit form to save checks.
                    //~ $start_link = "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='javascript:save_checks(0, \"{$moduleString}\");'>".SugarThemeRegistry::current()->getImage("start","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_START'])."</button>";
                         ...
                         $start_link = "<a type='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='javascript:save_checks(0, \"{$moduleString}\");'><i class='fa fa-angle-double-left fa-lg ' aria-hidden='true'></i></a>";
                    ...
                    $previous_link = "<a type='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='javascript:save_checks($previous_offset, \"{$moduleString}\");'><i class='fa fa-angle-left fa-lg ' aria-hidden='true'></i></a>";
                    ...
                    $start_link = "<a type='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='location.href=\"$start_URL\"; sListView.save_checks(0, \"{$moduleString}\");'><i class='fa fa-angle-double-left fa-lg ' aria-hidden='true'></i><a>";
					...
					$previous_link = "<a type='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='location.href=\"$previous_URL\"; sListView.save_checks($previous_offset, \"{$moduleString}\");'><i class='fa fa-angle-left fa-lg ' aria-hidden='true'></i></a>";
                } else {
                    $onClick = '';
					...
					$start_link = "<a type='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick=".$onClick."><i class='fa fa-angle-double-left fa-lg ' aria-hidden='true'></i></a>";
                    ...
                    $previous_link = "<a type='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick=".$onClick."><i class='fa fa-angle-left fa-lg ' aria-hidden='true'></i></a>";
            ...
            if($last_offset <= $current_offset) {
				...
				$end_link = "<a type='button' name='listViewEndButton' title='{$this->local_app_strings['LNK_LIST_END']}' disabled style='color:#6d6d6d'><i class='fa fa-angle-double-right fa-lg ' aria-hidden='true'></i></a>";
                ...
                $next_link = "<a type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' disabled style='color:#6d6d6d'><i class='fa fa-angle-right fa-lg ' aria-hidden='true'></i></a>";
            } else {
                if($this->multi_select_popup) { // nav links for multiselect popup, submit form to save checks.
					...
					$next_link = "<a type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}'  onClick='javascript:save_checks($next_offset, \"{$moduleString}\");'><i class='fa fa-angle-right fa-lg ' aria-hidden='true'></i></a>";
					...
					$end_link = "<a type='button' name='listViewEndButton' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='location.href=\"$end_URL\"; sListView.save_checks(\"end\", \"{$moduleString}\");'>					<i class='fa fa-angle-double-right fa-lg ' aria-hidden='true'></i></a>";
					...
					$next_link = "<a type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick='location.href=\"$next_URL\"; sListView.save_checks($next_offset, \"{$moduleString}\");'><i class='fa fa-angle-right fa-lg ' aria-hidden='true'></i></a>";
                } else {
                    $onClick = '';
                    ...
                    $next_link = "<a type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick=".$onClick."><i class='fa fa-angle-right fa-lg ' aria-hidden='true'></i></a>";
					...
					$end_link = "<a type='button' name='listViewEndButton' title='{$this->local_app_strings['LNK_LIST_END']}' onClick=".$onClick."><i class='fa fa-angle-double-right fa-lg ' aria-hidden='true'></i></a>";
                }
            }
...
$close_inline_img = SugarThemeRegistry::current()->getImage('close_inline', 'border=0', null, null, ".gif", $app_strings['LBL_CLOSEINLINE']);
            ...
            if($this->show_paging == true) {
                ...
                $html_text = '';
                $html_text .= "<tr class='pagination custom-pagination' role='presentation'>\n";
                ...
                if ($subpanel_def != null) {
							if($subpanel_def->_instance_properties['module']=="Activities"  || $subpanel_def->_instance_properties['module']=="History"){
							include_once('include/SubPanel/RightTapPanelTiles.php');
							$subpanelTiles = new RightTapPanelTiles($sugarbean);
							}else
							{
                    ...
                    if (isset($response['query']) && !empty($response['query'])){
						...
						$action_button_new = $subpanelTiles->get_buttons($subpanel_def, $response['query']);
                    }else{
                ...
                $this->xTemplate->assign("ACTION_BUTTON",$action_button_new);
                $this->xTemplate->assign("TOTAL_RECORD",$total_records);
            }
