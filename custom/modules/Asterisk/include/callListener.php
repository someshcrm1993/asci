...
//~ $query = "Select asterisk_ext_c,agent_id_c from users_cstm where id_c = '" . $_GET['profId'] . "'";
//~ $resultSet = $current_user->db->query($query, false);
//~ $row = $current_user->db->fetchByAssoc($resultSet);
$user_id=$current_user->id;
$query = "SELECT asterisk_ext_c,agent_id_c from users_cstm where id_c = '$user_id'";
//$GLOBALS['log']->fatal("profile id from lisener".$user_id."Hai");
$resultSet = $current_user->db->query($query, false);
$row = $current_user->db->fetchByAssoc($resultSet);
$agent = $row['asterisk_ext_c'];
//$GLOBALS['log']->fatal("AGENT IS".$agent);
...
$lastHour = date('Y-m-d H:i:s',time() - 1*60*30); // Keep the popup alive for 30 minutes
//$GLOBALS['log']->fatal("LAST HOUR IS".$lastHour);
$query = " SELECT * FROM ozonetel WHERE \"$lastHour\" < timestampCall AND (uistate IS NULL OR uistate != 'Closed') AND Callednumber='$agent' ";//CHANGED
...
//$query = " SELECT * FROM asterisk_log WHERE uistate IS NULL AND timestampHangup IS NULL AND (uistate IS NULL OR uistate != \"Closed\") AND (callstate != 'NeedID') AND (channel LIKE 'SIP/{$current_user->asterisk_ext_c}%' OR channel LIKE 'Local%{$current_user->asterisk_ext_c}%')";
...
while($row = $current_user->db->fetchByAssoc($resultSet)){
	...
	//$GLOBALS['log']->fatal("ARRAY RESULT IS".print_r($row,true));

	$item = array();
	//$item['asterisk_id'] = $row['asterisk_id'];
	$item['asterisk_id'] = $row['Sourceid'];//CHANGED
	...
	$tmpCallerID = trim($row['Callerid']);
		//$GLOBALS['log']->fatal("TEMPORARY CALLERID".	$tmpCallerID);
	...
	// if($row['direction'] == 'O') $item['phone_number'] = $row['CallerID']; // Edited By Ashraf
	$item['asterisk_name'] = $row['callerName'];
	//$item['asterisk_id'] = $row['asterisk_id'];
		$item['asterisk_id'] = $row['Sourceid'];//CHANGED
...
//$GLOBALS['log']->fatal("PHONE TO FIND".	$phoneToFind);
	...
	if(strlen($phoneToFind) > 5) {
        ...
        $selectPortion =  "SELECT distinct c.id as contact_id, first_name,	last_name,phone_work, phone_home, phone_mobile, phone_other, a.name as account_name, account_id "
            . "FROM contacts c left join accounts_contacts ac on (c.id=ac.contact_id AND c.deleted='0') left join accounts a on (ac.account_id=a.id) ";

        if( $row['contact_id'] ) {
            $wherePortion = " WHERE c.id='{$row['contact_id']}' and (c.deleted='0' OR ac.deleted='0' OR a.deleted='0')";
        ...
        // We only do this expensive query if it's not already set!
            ...
            $wherePortion .= sprintf($sqlReplace, "phone_mobile", $phoneToFind) . ") and (c.deleted='0' OR ac.deleted='0' OR a.deleted='0')";
        }

        $queryContact = $selectPortion . $wherePortion;
        //$queryContact = "SELECT c.id as contact_id, first_name,last_name,phone_work, phone_home, phone_mobile, phone_other, a.name as account_name, account_id FROM contacts c left join accounts_contacts ac on (c.id=ac.contact_id) left join accounts a on (ac.account_id=a.id) WHERE phone_mobile = '9731847617'";
		...
		while($contactRow = $current_user->db->fetchByAssoc($innerResultSet)){
			...
			//$GLOBALS['log']->fatal("FIRST NAME".$contactRow['first_name']);
            ...
            if( empty( $row['contact_id'] ) && !$isMultipleContactCase ) {
                ...
                //$insertQuery = "UPDATE asterisk_log SET contact_id='$tempContactId' WHERE call_record_id='$tempCallRecordId'";
                          $insertQuery = "UPDATE ozonetel SET contact_id='$tempContactId' WHERE call_record_id='$tempCallRecordId'";
        ...
        /*if( !isset($found['company']) )
        {
            ...
            $item['callerid'] = $row['opencnam']; // $queryContact ; // 
        }*/
...
EOT1;
        $createNewContactLink = '<a class="asteriskActionsLinks"  href="index.php?module=Contacts&action=EditView&phone_work=' . $phoneToFind .'" title="Create a new contact with this phone number">New Contact</a>';
        $createNewCase = '<a class="asteriskActionsLinks"  href="index.php?module=Cases&action=EditView&contact_id=' . '4a3178e9-3a01-49c3-dc45-54e5e8d92b55' .'" title="Create a new contact with this phone number">New Case</a>';
