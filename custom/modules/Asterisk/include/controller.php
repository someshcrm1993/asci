...
if( $_REQUEST['action'] == "memoSave") {
	...
	**/
	...
	$GLOBALS['log']->fatal("for subject from dailin.js".$_POST["subject"]);
	//$focus->name = $subject;		
	//$focus->save();
	$query = "Update calls set description = '" . $_POST["description"] . "' where id = '" . $_POST["call_record"] . "'";
	$resultSet = $cUser->db->query($query, false);
	if($cUser->db->checkError()) {
		trigger_error("Update Calls note failed: $query");
	}
}
	...
	$GLOBALS['log']->fatal("call record id".$callRecord );
	$GLOBALS['log']->fatal("uistate is".$uiState );
	$query = "update ozonetel set uistate=\"$uiState\" where call_record_id=\"$callRecord\""; 
	//echo $query;
    ...
    $callRecord = preg_replace('/[^a-z0-9\-\. ]/i', '', $_REQUEST['call_record']); // mysql_real_escape_string($_REQUEST['call_record']);
    $query = "update ozonetel set contact_id=\"$contactId\" where call_record_id=\"$callRecord\"";
