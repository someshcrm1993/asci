...
// * Parts of this code are (c) 2011 Blake Robertson http://call
...
function checkForNewStates(){
		...
		// $.getJSON('callListener.php', function(data){checkData(data);});
}
...
// Testing Domload event
document.addEventListener("DOMContentLoaded", function(event) {
	if(location.href.indexOf('action=Login') == -1){
                $('<div id="asterisk_ajaxContent" style="display:none;"></div>').prependTo('#main');
                // checkForNewStates();
                setTimeout(checkForNewStates, 12000);
        }
});
