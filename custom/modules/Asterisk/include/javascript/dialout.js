...
$(document).ready(function()
{
	...
	$('.phone,#phone_work,#phone_other,.asterisk_phoneNumber,td[type=phone]').each(function()
	{
...
//alert(phoneNr);
        ...
        if(phoneNr.length > 1 && !/(class="phone"|id="?#phone)/.test($(this).html()))
		{
			...
			$('.asterisk_placeCall', this).click(function()
			{
				...
				//~ var ProfId = $('.profileactions-profile a').attr('href');
			//~ idd = ProfId.split("record=");
			//~ //alert(idd);
			//~ //alert(idd[1]);
			//~ profileid=idd[1];
			//~ //alert(profileid);
                                //~ $.ajax({
						//~ url: 'callcreatekookoo.php',
						//~ type:'POST',
						//~ data:{
							//~ phoneNr : phoneNr,
							//~ profileid : profileid,
