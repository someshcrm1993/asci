       ...
       if( $buffer === FALSE ) 
		{
			...
			if ($buffer == "\r\n") { // handle partial packets
				...
				// parse the event and get the result hashtable
                                //logLine("\n Event: " . $event . print_r($event) . " \n");
				$e             = getEvent($event);
                                //logLine("\n Dumping Values Start \n");
				...
				if ($e['Event'] == 'Dial' && $e['SubEvent'] != 'End') //Asterisk Manager 1.1 returns 2 dial events with a begin and an end subevent, by excluding the subevent end we won't get multiple records for the same call.
					...
					$tmpCallerID = trim($e['CallerID']); //Asterisk Manager 1.0 $e['CallerID'] // $tmpCallerID = trim($e['CallerIDNum']);
					...
					{					
						//Asterisk Manager 1.1 (If the call is internal, this will be skipped)
                                                logLine("(preg_match(".$asteriskMatchInternal.", ".$e['Source'].") && !preg_match(".$asteriskMatchInternal.", ".$e['Destination']."))"); // $e['Channel']
						if (preg_match($asteriskMatchInternal, $e['Source']) && !preg_match($asteriskMatchInternal, $e['Destination'])) {
							$query         = sprintf("INSERT INTO asterisk_log (asterisk_id, call_record_id, channel, remote_channel, callstate, direction, CallerID, timestampCall) VALUES('%s','%s','%s','%s','%s','%s','%s',%s)", $e['DestUniqueID'], $callRecordId, $e['Source'], $e['Destination'], 'NeedID', 'O', $tmpCallerID, 'NOW()');
                                                        logLine($query . "\n");
							...
							$query         = sprintf("INSERT INTO asterisk_log (asterisk_id, call_record_id, channel, remote_channel, callstate, direction, CallerID, timestampCall, asterisk_dest_id) VALUES('%s','%s','%s','%s','%s','%s','%s',%s,'%s')", $e['SrcUniqueID'], $callRecordId, $e['Destination'], $e['Channel'], 'Dial', 'I', $tmpCallerID, 'NOW()', $e['DestUniqueID']);
				...
				//
				//Asterisk Manager 1.1
                                // if ($outflag) logLine("\n This is out bound. This event is " . $e['Event'] . " and Caller Id is " . $e['CallerID'] . "\n");
				if ($e['Event'] == 'Newcallerid') {
                                    logLine("\n Found new Caller id . ");
					$id          = $e['Uniqueid'];
					$tmpCallerID = trim($e['CallerID']); // CallerIDNum - Edited by Ashraf
					//echo ("* CallerID is: $tmpCallerID\n");
                                        logLine("(".strlen($calloutPrefix)." > 0) && (".strpos($tmpCallerID, $calloutPrefix)." === 0)");
