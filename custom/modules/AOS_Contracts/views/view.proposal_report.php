...
class AOS_ContractsViewproposal_report extends SugarView {
    ...
    function display()
	{
		...
		if (!isset($_POST['proposal_status']) && !isset($_POST['outcome_status'])) {
			$where .= 'AND opportunities_cstm.asci_proposal_outcome_c = "Under negotiation" && opportunities_cstm.asci_proposal_outcome_c = "Others"';
		}
...
<html lang="en">
	...
	<style>
		...
		while($row_enquiry=$query->fetch()){
			...
			$data .='<td><a href = "index.php?module=Project&action=DetailView&record='.$row_enquiry['id'].'">'.$row_enquiry['title'].'</a></td>';
			$data .='<td>'.$row_enquiry['lead_faculty1'].'</td>';
			$data .='<td>'.$row_enquiry['lead_faculty2'].'</td>';
			$data .='<td>'.$row_enquiry['date_submission_approved_prp_c'].'</td>';
			...
			$data .='<td>'.number_format($row_enquiry['fee_quoted_c'], 2).'</td>';
