    ...
    public function preDisplay() {
        ...
        <script>
             $(document).ready(function(){
				...
				$('#project_contacts_2_name_basic').on('change blur paste',function(){
                    
					copyprogramcode();
					 
                }); 
				
				$("#Contactsbasic_searchSearchForm").mouseup(function(){
					copyprogramcode();
				});
				/*$("#Contactsbasic_searchSearchForm").mousedown(function(){
					copyprogramcode();
				});*/
				$('#search_form_clear').on('click',function(){
					document.getElementById("search_form_clear").click(); 
					$('#programme_id_c_basic').val('');
					$('#project_contacts_2project_ida_basic').val("");
					 setTimeout(function(){ if($('#project_contacts_2project_ida_basic').val()=="") {copyprogramcode(); } }, 1000);	
                }); 
				
			}); 			
			function copyprogramcode(){
					var pid = $('#project_contacts_2project_ida_basic').val();
                   ...
                   if(pid != '')
                   {
                        $.ajax({url:'popupproginfo.php', data:{pid1:pid},type:'GET',dataType:'json',success:function(result)
                        {
							...
							$('#project_contacts_2_name_basic').focusout();  
                        }
                        });
                   }
                   else
