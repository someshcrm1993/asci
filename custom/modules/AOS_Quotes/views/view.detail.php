...
class AOS_QuotesViewDetail extends ViewDetail {
	...
	function display(){
		...
		// $participants = json_decode(html_entity_decode($this->bean->participant_list_c));
		...
		// 						 }, $participants
		// 				);
		// $this->bean->participant_list_c = '<ul class="list-group">'.implode('', $participants).'</ul>';
		$pinvoiceButton = 'invoice';
		// if ($this->bean->programme_type_c == "Announced") {
		// 		$pinvoiceButton = 'invoice';
		// }else if ($this->bean->programme_type_c == "ICTP-On Campus") {
		// 		$pinvoiceButton = 'pinvoiceictpon';
		// }else if ($this->bean->programme_type_c == "ICTP-Off Campus") {
		// 		$pinvoiceButton = 'pinvoiceictpoff';
		// }
		...
		if($role == 'PO(M1,M2,M3)' || $current_user->id == '5eb35c29-8944-28f5-3f1c-59436623fb2e'){
			$po = true;
		}else{
			$po = false;
		}
		$this->ss->assign('po',$po);
		if($_REQUEST['message']){
		...
		$js = <<<EOD
			<script>
				$(document).ready(function(){
					...
					//adding print button
                	//$('#detail_header_action_menu').before('<a target="_blank" href="{$sugar_config["site_url"]}index.php?module=AOS_Quotes&action=sugarpdf&sugarpdf={$pinvoiceButton}&record={$this->bean->id}" class="glyphicon glyphicon-print fa-2x" style="margin-right: 14px;"></a>');
    				
    				if('{$this->bean->invoice_status}' == 'Invoiced'){
    					$('.moduleTitle').prepend('<h6 style="color:red">This proforma invoice is already converted into invoice.</h6>');
    				}
              });
