...
class AORReportsDashlet extends Dashlet {
    ...
    public function displayOptions() {
      //modified by manasa
    global $db,$beanList;
    //$GLOBALS['log']->fatal(print_r($this->report,true),"Report Bean");

    $report_id = $this->report->id;
    $query_report_info = "SELECT field from aor_conditions where aor_report_id='$report_id' and value_type='Multi'";
    $result_report_info = $db->query($query_report_info);
    $row_report_info = $db->fetchByAssoc($result_report_info);
    $field = $row_report_info['field'];
    //$GLOBALS['log']->fatal($field,"Multi Select Value from Dashlet");
    $report_name = $this->report->report_module;
    if($report_name !=''){
    $module = new $beanList[$report_name]();
  $fields_definition = $module->field_defs[$field]['options'];
}
        ob_start();
        ...
        //modified by manasa
      $optionsSmarty->assign('enum_list', $app_list_strings[$fields_definition]);
