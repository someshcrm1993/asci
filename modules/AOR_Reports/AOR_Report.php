...
class AOR_Report extends Basic {
    ...
    function build_report_chart($chartIds = null, $chartType = self::CHART_TYPE_PCHART){
        ...
        $GLOBALS['log']->fatal($query,"Build Report Query");
        ...
        while($row = $this->db->fetchByAssoc($result, false))
        {
            foreach($fields as $name => $att){
                ...
                $currency_id = isset($row[$att['alias'].'_currency_id']) ? $row[$att['alias'].'_currency_id'] : '';
					//$GLOBALS['log']->fatal($att['function'],"Function");
                switch ($att['function']){
                    case 'COUNT':
                        break;
                    default:
        ...
        if($offset >= 0){
            ...
            $html .="<td colspan='18'>
                       <table class='paginationTable' border='0' cellpadding='0' cellspacing='0' width='100%'>
                        <td style='text-align:left' ><H3><a href=\"javascript:void(0)\" class=\"collapseLink\" onclick=\"groupedReportToggler.tan(arg)oggleList(this);\"><img border=\"0\" id=\"detailpanel_1_img_hide\" src=\"themes/SuiteR/images/basic_search.gif\"></a>$moduleFieldByGroupValue</H3></td>
			...
			$html .="
			<a  id='listViewStartButton_top' name='listViewStartButton' title='Start' disabled='disabled' style='color:#6d6d6d'>
			<i class='fa fa-angle-double-left fa-2x pagination-icon-bottom-disabled' aria-hidden='true'></i>
			</a>&nbsp;&nbsp;
			<a id='listViewPrevButton_top' name='listViewPrevButton' title='Previous' disabled='disabled' style='color:#6d6d6d'>
			<i class='fa fa-angle-left fa-2x pagination-icon-bottom-disabled' aria-hidden='true'></i>
			 ...
			 $html .="
			<a id='listViewStartButton_top' name='listViewStartButton' title='Start' onclick='changeReportPage(\"".$this->id."\",0,\"".$group_value."\",\"".$tableIdentifier."\")'>
			<i class='fa fa-angle-double-left fa-2x pagination-icon-bottom' aria-hidden='true'></i>
			</a>&nbsp;&nbsp;
			<a  id='listViewPrevButton_top' name='listViewPrevButton' class='button' title='Previous' onclick='changeReportPage(\"".$this->id."\",".$previous_offset.",\"".$group_value."\",\"".$tableIdentifier."\")'>
			...
			$html .="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id='listViewNextButton_top' name='listViewNextButton' title='Next' onclick='changeReportPage(\"".$this->id."\",".$next_offset.",\"".$group_value."\",\"".$tableIdentifier."\")'>
			<i class='fa fa-angle-right fa-2x pagination-icon-bottom' aria-hidden='true'></i>
			</a>&nbsp;&nbsp;
			<a id='listViewEndButton_top' name='listViewEndButton' title='End' onclick='changeReportPage(\"".$this->id."\",".$last_offset.",\"".$group_value."\",\"".$tableIdentifier."\")'>
			...
			";
                ...
                //~ $html .="<button type='button' id='listViewNextButton_top' name='listViewNextButton' title='Next' class='button'  disabled='disabled'>
                    ...
                    //~ </button>";
			$html .="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id='listViewNextButton_top' name='listViewNextButton' title='Next'   disabled='disabled' style='color:#6d6d6d'><i class='fa fa-angle-right fa-2x pagination-icon-bottom-disabled' aria-hidden='true'></i>
			</a>&nbsp;&nbsp;

			<a id='listViewEndButton_top' name='listViewEndButton' title='End' disabled='disabled' style='color:#6d6d6d'>
    ...
    function build_report_csv(){
		...
		//$GLOBALS['log']->fatal(print_r($csv,true),"CSV");
        ob_clean();
        header("Pragma: cache");
    ...
    function build_report_query($group_value ='', $extra = array()){
		...
		//$GLOBALS['log']->fatal($query_where,"Query where");
        if(isset($query_array['group_by'])){
            $query_group_by = '';
            foreach ($query_array['group_by'] as $group_by){
                $query_group_by .=  ($query_group_by == '' ? 'GROUP BY ' : ', ').$group_by;
		...
		//$GLOBALS['log']->fatal($query_array['sort_by'],"Sorting");
        if(isset($query_array['sort_by'])){
            $query_sort_by = '';
            foreach ($query_array['sort_by'] as $sort_by){
                $query_sort_by .=  ($query_sort_by == '' ? 'ORDER BY ' : ', ').$sort_by;
    ...
    function build_report_query_select($query = array(), $group_value =''){
        ...
        if($beanList[$this->report_module]){
            ...
            while ($row = $this->db->fetchByAssoc($result)) {
				...
				//Added By Saurabh S to add Main Group Field Into query
								 if($field->group_display == 1){
								 		$query['group_by'][] = $field->format ? str_replace('(%1)', '(' . $select_field . ')', preg_replace(array('/\s+/', '/Y/', '/m/', '/d/'), array(', ', 'YEAR(%1)', 'MONTH(%1)', 'DAY(%1)'), trim(preg_replace('/[^Ymd]/', ' ', $field->format)))) : $select_field;
								 }
...
//                            $table_alias = 'opportunities:assigned_user_link:SecurityGroups' ;
					...
					//$GLOBALS['log']->fatal($condition->value,"Value Type");
                    switch ($condition->value_type) {
                        ...
                        case "Period":
                          ...
                          if(($condition->value=='this_year') || ($condition->value == 'last_year'))
                            {
                            $value = 'AND'.' '. 'EXTRACT(YEAR FROM'.''. ($field).')'.'= '.'"' . getPeriodDate($params)->format('Y') . '"';
                            $query['where'][] = $value;
                            $where_set = true;
							}
							else if($condition->value =='last_month')
							{
								 $value = 'AND'.' '. 'EXTRACT(MONTH FROM'.''. ($field).')'.'= '.'"' . getPeriodDate($params)->format('m') . '"';
                            $query['where'][] = $value;
                            $where_set = true;
							}
							else if($condition->value =='last_week')
							{
								
								 $value = 'AND'.' '. 'DATE('. ($field).')'.'= '.'"' .'DATE_SUB(CURDATE(), INTERVAL 1 WEEK)' . '"';
                            $query['where'][] = $value;
                            $where_set = true;
							}
							...
							{
								$date = date("Y-m-d");
								$date_minus_7days = date('Y-m-d', strtotime("-7 days"));
								$value = 'AND'.' '. 'DATE('.($field).')'.' '.'BETWEEN "'.$date_minus_7days.'" AND "'.$date.'"';
								$query['where'][] = $value;
								$where_set = true;
							}
							else if($condition->value =='this_quarter')
							{
									$value = 'AND'.' '. 'DATE('. ($field).')'."BETWEEN MAKEDATE(YEAR(CURDATE()), 1) + INTERVAL QUARTER(CURDATE()) QUARTER - INTERVAL 1 QUARTER AND MAKEDATE(YEAR(CURDATE()), 1) + INTERVAL QUARTER(CURDATE()) QUARTER - INTERVAL 1 DAY";
									$query['where'][] = $value;
								$where_set = true;
							}
							else if($condition->value =='last_quarter')
							{
									$value = 'AND'.' '. 'DATE('. ($field).')'."BETWEEN MAKEDATE(YEAR(CURDATE()), 1) + INTERVAL QUARTER(CURDATE())-2 QUARTER AND MAKEDATE(YEAR(CURDATE()), 1) + INTERVAL QUARTER(CURDATE())-1 QUARTER - INTERVAL 1 DAY ";
									$query['where'][] = $value;
								$where_set = true;
							}
							else if($condition->value =='today')
							{
								$params = $condition->value;
							$date = '"'.date('Y-m-d',strtotime('+480 minutes',strtotime($params))).'%'.'"';
							$value = 'AND'.' '. $field.' '.'LIKE('."$date".')';
							$query['where'][] = $value;
                            $where_set = true;
							}
							else if($condition->value =='yesterday')
							{
								$value = 'AND'.' '.'DATE('. ($field).')'.'= '.' '."CURDATE() - INTERVAL 1 DAY";
							$query['where'][] = $value;
                            $where_set = true;
							}
							else {
                       ...
                       if($condition->operator=='Equal_To')
                        {
							...
							$query['where'][] = $value;
                            $where_set = true;
							//$GLOBALS['log']->fatal($value,"Value in datetime");
							}
							else
							{
