...
class AOR_ReportsViewDetail extends ViewDetail {
    ...
    public function preDisplay() {
        ...
        parent::preDisplay();
        $report_id = $this->bean->id;
        //~ echo $report_id;exit;
        //added on 22May2017 'if report having chart then only  call the build report chart else not'.
        $sql = "SELECT id FROM aor_charts WHERE aor_report_id = '".$this->bean->id."' AND deleted = 0";
        $result = $db->query($sql);
...
EOD;
        ...
        // commented on 22May2017 because 'build report chart should not call if report not having any chart'
        // $chartsHTML = $this->bean->build_report_chart(null, AOR_Report::CHART_TYPE_RGRAPH);
        //
        // $chartsPerRow = $this->bean->graphs_per_row;
        //
