...
class RegionWiseSalesStatusDashlet extends DashletGenericChart
{
    ...
    )
    {
        ...
        if(empty($options['title']))
            $options['title'] = translate('LBL_RGraph_REGION_FORM_TITLE', 'Home');
 ...
 $chart = <<<EOD
    ...
    <script type="text/javascript">
      ...
      function drawRegionsMap() {		
		...
		var chart1 = new google.visualization.GeoChart(document.getElementById('regionchart-colors'));
      ...
      <body>
		<div id="regionchart-colors" style="width: 100%; height: 433px; align=center;"></div>
	  </body>
EOD;
        return $chart;
    }
...
*/
    ...
    protected function constructQuery()
    {
		...
		$query = "SELECT A.billing_address_state as region, sum(amount_usdollar) AS total 
FROM opportunities 
JOIN accounts_opportunities AO ON AO.opportunity_id=opportunities.id
JOIN accounts A ON A.id=AO.account_id 
WHERE opportunities.sales_stage IN('Closed Won') and A.billing_address_country='India' and opportunities.deleted=0 and A.deleted=0 and AO.deleted=0 GROUP BY opportunities.sales_stage, A.billing_address_state";
    ...
    protected function prepareChartData($data,$currency_symbol, $thousands_symbol)
    {
        ...
        foreach($data as $i)
        {
            ...
            $chart['labelsAndValues'][]=$i['region'].' ('.$currency_symbol.(int)$i['total'].$thousands_symbol.')';
            $chart['labels'][]=$i['region'];
